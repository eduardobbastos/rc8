#!/usr/bin/env python3
"""
RC8 - Sincronizador Completo de Músicas e Metadados (Planilha -> GitHub Audio & JSON)
...
print("=" * 60)
print("📻 RC8 - SINCRONIZADOR DE MÚSICAS & METADADOS (GITHUB AUDIO)")
print("=" * 60)
...
def find_existing_audio_file(title, artist, yt_id):
    ...
    for f in existing_files:
        ...
        if title_clean and title_clean in f_clean:
            return f
            
    return None

def download_audio_from_youtube(yt_url, desired_filename):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        try:
            ...
            print(f"❌ Erro ao baixar YouTube: {err}")
            return None
...
def main():
    ...
    for i, row in enumerate(rows[1:], start=1):
        ...
        # Verifica se já temos o áudio na pasta audio/
        audio_file = find_existing_audio_file(title, artist, yt_id)

        if not audio_file and yt_link and ('youtube.com' in yt_link or 'youtu.be' in yt_link):
            print(f"\n⬇️ [{i}] Baixando para audio/: {title} ({artist})...")
            audio_file = download_audio_from_youtube(yt_link, preferred_filename)
        elif audio_file:
            print(f"⚡ [{i}] Áudio já existente em audio/: {audio_file}")
    ...
    # Salva no arquivo de metadados data/playlist.json
    ...
    print("\n" + "=" * 60)
    print(f"🎉 SUCESSO! {len(playlist)} faixas sincronizadas no arquivo:")
    print(f"📄 {json_path}")
    print("=" * 60)
